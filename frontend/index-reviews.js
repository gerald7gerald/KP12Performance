document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Clip the window + set up the container as a flex row FIRST, before we
    // even know whether there are reviews yet. Previously this only ran
    // inside the "has reviews" branch below, so the empty-state fallback
    // card (shown when there are 0 reviews) had nothing constraining its
    // width and could overflow the viewport on mobile — that's the actual
    // bug, not a missing/mismatched CSS class.
        ...
        if (!reviews || reviews.length === 0) {
            container.innerHTML = `
                <div style="min-width:100%;flex-shrink:0;box-sizing:border-box;
                    ...
                    <div style="color:var(--athletics,#3D9EFF);font-size:18px;letter-spacing:3px;margin-bottom:14px;">★★★★★</div>
                    <p style="font-family:'Work Sans',sans-serif;font-style:italic;font-size:15px;
                              line-height:1.7;color:#F5F4F0;margin:0 0 18px;
                              word-wrap:break-word;overflow-wrap:break-word;white-space:normal;">
                        "Be the first athlete to leave a review and share your experience."
                    </p>
                    <p style="font-family:'JetBrains Mono',monospace;font-size:11px;
                              letter-spacing:0.06em;color:#8C8F96;margin:0;">— Your Name Here</p>
                </div>`;
            return;
        }
